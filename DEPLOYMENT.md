...
Create or configure a GitHub App with this callback URL (use the same public
domain you use to open `/admin`; the active domain is shown below):
...
For a GitHub App, set the repository permission **Contents** to **Read and
write**, install the App on `FlatWalnut/firefly-blog`, and approve any pending
permission update. The non-secret repository settings are in `wrangler.jsonc`.
`GITHUB_AUTH_TYPE=github-app` makes the authorization URL omit OAuth scopes;
the App's fine-grained permissions control access. The token remains in an
encrypted, HttpOnly session cookie and is never returned to browser JavaScript.
The repository owner is restricted by `GITHUB_ALLOWED_LOGIN`.
